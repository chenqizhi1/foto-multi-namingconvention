...
class AppDialog(QtGui.QWidget):
    ...
    def __init__(self):
        ...
        self.fields = None
        self.ctx = None
        self.extension = {'photoshop':'psd', 'maya':'mb', 'nuke':'nk'}
        ...
        # self.log.warn(self.template_definitions)

        self.custom_entity_name_remap = self._app.get_setting("custom_entity_name_remap")
        # self.log.warn(self.custom_entity_name_remap)
    ...
    def update_tk_templates(self):
        ...
        if app_name:
            self.app_name = app_name.lower().replace(' ', '').replace('tk-', '')
        else:
            self.app_name = self.ui.appComboBox.currentText().lower()
        # self.log.warn(app_name)
        ...
        # self.log.info(context_dict['entity']['type'])
        if context_dict['entity']['type'] == 'Asset':
            if self.app_name == 'photoshop': 
                self.regex = ['photoshop_asset_work', 'asset_work_area_photoshop']
            elif self.app_name == 'maya':
                self.regex = ['asset_work_area_maya', 'maya_asset_work']
            elif self.app_name == 'nuke':      
                self.regex = ['nuke_asset_work', 'asset_work_area_nuke']
        elif context_dict['entity']['type'] == 'Shot':
            if self.app_name == 'photoshop': 
                self.regex = ['photoshop_shot_work', 'shot_work_area_photoshop']
            elif self.app_name == 'maya':      
                self.regex = ['shot_work_area_maya', 'maya_shot_work']
            elif self.app_name == 'nuke':      
                self.regex = ['nuke_shot_work', 'shot_work_area_nuke']
        ...
        for k, v in templates.iteritems():
            for x in self.regex:
                if re.search(x, k):
                    active_templates[k] = v
                    # self.log.info(v.definition)
        
        # self.log.info(active_templates)
        ...
        for key in active_keys:
            key_title = key.replace(entity_type, '').replace('_', ' ').strip().title()
            # self.log.warn(key_title)
    ...
    def define_tk_context(self, context):
        ...
        # self.log.error(self.context.to_dict())
    ...
    def update_tk_context(self):
        ...
        # self.log.info(self.context.to_dict())
        ...
        # self.log.info(self.app.)
        self.ctx = self._app.sgtk.context_from_entity(typ, idd)
        # self.log.info('asdsadasdasdada')
        # self.log.info(self.ctx.to_dict())
    ...
    def update_template_output_paths(self):
        ...
        # self.log.warn(self.template.definition)
        ...
        # self.log.warn(self.fields)

        missing_keys = self.template.missing_keys(self.fields)

        self.update_extra_tokens_widgets(self.template, missing_keys)

    def _template_work_area(self):
        try:
            work_area = [x for x in self.regex if x == u'%s_work_area_%s' % (self.context.entity['type'].lower(), self.app_name)]
            for x, y in self._app.context.sgtk.templates.iteritems():
                if work_area:
                    if re.search(work_area[0], x):
                        # self.log.info(work_area)
                        # self.log.warn(x)
                        work_area_template = y
                        # self.log.warn(work_area_template.definition)
        except Exception, e: self.log.error(e)
        return work_area_template
    ...
    def update_extra_tokens_widgets(self, template, missing_keys):
        ...
        row_cnt = 0
        context_dict = self.context.to_dict()
        name = "%s_%s" % (context_dict['entity']['name'], context_dict['task']['name'])
        # self.log.info(name)
        if not keys:
                ...
                if key == 'name':
                    ...
                    try:
                        if os.path.exists(path):
                            ...
                            # self.log.info(files)
                            if files:
                                work_files = [s for s in files if os.path.isfile(os.path.join(path, s))]
                                # self.log.info(work_files)
                                if work_files:
                                    version = len([x for x in work_files if x.split('.')[-1].lower() == self.extension[self.app_name]]) + 1
                                else: version = 1
                            else:
                                version = 1
    ...
    def update_template_file_path(self):
        ...
        # self.log.warn(self.fields)
        # self.log.info(self.template.definition)
        # for i in dir(self.template): self.log.error(i)
        try:
